...
const PatientDetails = () => {
  ...
  const handleDownloadReport = () => {
        if (patientData) {
          ...
          // Function to add a single column row
          const addSingleColumnRow = (label, value, unit = "", isHeader = false) => {
            const margin = 20; // Margin at the bottom of the page
            const pageHeight = doc.internal.pageSize.height;
          ...
          order_details.forEach((detail, index) => {
            ...
            addSingleColumnRow("Collection Date Time:", detail.collection_date_time);
            yPos += 6;
          });
    ...
    {order_details.map((orderDetail, index) => (
      <div key={index} className="bg-white p-4">
        ...
        <p>
          ...
          {new Date(orderDetail.collection_date_time).toLocaleDateString()}
        </p>
      </div>
    ))}
  </div>
